...
import { Anatomy3D } from '../../../ui/Anatomy3D'
...
export function VisualizarProntuarioView({ onPageChange }: VisualizarProntuarioViewProps) {
    ...
    const [showAnatomy3D, setShowAnatomy3D] = useState(false);
    ...
    const patientProfile = {
        ...
        foto: 'https://ui-avatars.com/api/?name=Maria+Silva&background=0039A6&color=fff',
    }

    const getAge = (birthDateBr: string) => {
        ...
        <div className="space-y-6 animate-in fade-in duration-500 pb-10">
            ...
            <div className="bg-white dark:bg-app-card-dark rounded-[24px] p-8 shadow-sm border border-gray-100 dark:border-gray-800/50 space-y-8 relative">
                ...
                <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                    <div className="flex items-center gap-6">
                        <img
                            src={patientProfile.foto}
                            alt={patientProfile.nome}
                            className="h-16 w-16 rounded-full object-cover border border-gray-100 dark:border-gray-800 shadow-sm"
                        />
                        <div className="space-y-1">
                            <h1 className="text-2xl font-normal text-gray-900 dark:text-white tracking-tight">{patientProfile.nome}</h1>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-4 border-t border-gray-100 dark:border-gray-800/50">
                    <div>
                        <p className="text-[11px] uppercase tracking-wider text-app-text-muted">Data de nascimento</p>
                        <p className="text-sm text-gray-900 dark:text-white">{patientProfile.dataNascimento}</p>
                    </div>
                    <div>
                        <p className="text-[11px] uppercase tracking-wider text-app-text-muted">Idade</p>
                        <p className="text-sm text-gray-900 dark:text-white">{getAge(patientProfile.dataNascimento)} anos</p>
                    </div>
                    <div>
                        <p className="text-[11px] uppercase tracking-wider text-app-text-muted">CPF</p>
                        <p className="text-sm text-gray-900 dark:text-white">{patientProfile.cpf}</p>
                    </div>
                    <div>
                        <p className="text-[11px] uppercase tracking-wider text-app-text-muted">Telefone</p>
                        <p className="text-sm text-gray-900 dark:text-white">{patientProfile.telefone}</p>
                    </div>
                    <div>
                        <p className="text-[11px] uppercase tracking-wider text-app-text-muted">Email</p>
                        <p className="text-sm text-gray-900 dark:text-white">{patientProfile.email}</p>
                    </div>
                    <div className="md:col-span-3">
                        <p className="text-[11px] uppercase tracking-wider text-app-text-muted">Endereço</p>
                        <p className="text-sm text-gray-900 dark:text-white">{patientProfile.endereco}</p>
                    </div>
                </div>
            ...
            <div className="bg-white dark:bg-app-card-dark rounded-[24px] p-8 shadow-sm border border-gray-100 dark:border-gray-800/50 space-y-6">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    ...
                    <Button
                        variant="outline"
                        onClick={() => setShowAnatomy3D(prev => !prev)}
                        className="h-10 px-4 rounded-[10px] border-gray-100 dark:border-gray-800 text-gray-900 dark:text-white font-normal hover:bg-app-bg-secondary dark:hover:bg-white/5 flex items-center gap-2 transition-all bg-white dark:bg-transparent shadow-sm"
                    >
                        <Box className="h-4 w-4 text-app-text-muted" />
                        {showAnatomy3D ? 'Ocultar anatomia 3D' : 'Visualizar anatomia 3D'}
                    </Button>
                </div>
