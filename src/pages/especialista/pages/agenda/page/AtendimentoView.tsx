...
import { registrarRetornoAtendimento } from '@/services/retorno.service'
...
function AtendimentoContent({ onPageChange, patientName }: AtendimentoViewProps) {
  ...
  const saveReturnRequest = async () => {
    ...
    if (dueDate) {
      await registrarRetornoAtendimento({
        paciente: safePatientName,
        profissional: 'Especialista responsável',
        especialidade: 'Clínica Integrativa',
        dataAtendimento: now.toISOString().split('T')[0],
        proximoRetorno: dueDate.toISOString().split('T')[0],
        returnOption,
