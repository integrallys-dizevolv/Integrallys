...
const MOCK_PATIENTS = MOCK_PRESCRICOES_PACIENTES
const MOCK_PRODUCTS = MOCK_PRESCRICOES_PRODUTOS
const MOCK_PROFESSIONALS = MOCK_PRESCRICOES_USUARIOS
...
export function EditarPrescricaoModal({ isOpen, onClose, prescricao, onSave }: EditarPrescricaoModalProps) {
    ...
    useEffect(() => {
        if (prescricao) {
            ...
            setPaymentMethod(
                prescricao.tipo_venda === 'consumo' ||
                    (prescricao.generator ?? '').toLowerCase().includes('consumo')
                    ? 'consumo'
                    : '',
            )
        }
    ...
    useEffect(() => {
        if (paymentMethod === 'consumo') {
            const itensRecalculados = cartItems.map((item) => {
                const produto = MOCK_PRODUCTS.find((p) => p.id === item.productId)
                const displayUnitPrice = produto?.costPrice ?? item.unitPrice
                return {
                    ...item,
                    displayUnitPrice,
                    displayTotal: displayUnitPrice * item.quantity,
                }
            })
            setDisplayItems(itensRecalculados)
            return
        }

        setDisplayItems(cartItems.map((item) => ({
            ...item,
            displayUnitPrice: item.unitPrice,
            displayTotal: item.unitPrice * item.quantity,
        })))
    }, [paymentMethod, cartItems])

    // Computed Values
    const isConsumo = paymentMethod === 'consumo'
    const subtotal = displayItems.reduce((acc, item) => acc + item.displayTotal, 0)
    ...
    const handleSave = async () => {
        ...
        const persistedItems: CartItem[] = cartItems.map((item) => {
            const produto = MOCK_PRODUCTS.find((p) => p.id === item.productId)
            const unitPrice = isConsumo ? (produto?.costPrice ?? item.unitPrice) : item.unitPrice
            return {
                ...item,
                unitPrice,
        ...
        const totalPersistido = persistedItems.reduce((acc, item) => acc + item.total, 0)
        const totalFinal = isConsumo ? totalPersistido : Math.max(0, totalPersistido - discountAmount)
        ...
        const updatedPrescricao: Prescription = {
            ...
            totalValue: totalFinal > 0 ? totalFinal : prescricao.totalValue,
            status: paymentMethod ? 'Convertida' : status,
            generator: isConsumo ? 'Consumo (sem caixa)' : (prescricao.generator ?? 'Manual'),
            tipo_venda: isConsumo ? 'consumo' : 'normal',
            items: persistedItems,
        }

        const shouldApplyEstoque = Boolean(paymentMethod) && persistedItems.length > 0 && (isConsumo || prescricao.status !== 'Convertida')
        if (shouldApplyEstoque) {
            const sellerName = MOCK_PROFESSIONALS.find((p) => p.id === selectedSellerId)?.name || prescricao.specialistName
            const patientName = MOCK_PATIENTS.find((p) => p.id === selectedPatientId)?.name || prescricao.patientName
            ...
            if (isConsumo) {
                persistedItems.forEach((item) => {
                    registrarConsumoInterno({
                        ...
                        origem: 'edicao-prescricao',
                    })
                })
            }

            const shouldLaunchCaixa = !isConsumo && prescricao.status !== 'Convertida'
            if (shouldLaunchCaixa) {
                const caixaMethod =
                    paymentMethod === 'pix'
                        ? 'pix'
                ...
                await appendCaixaTransaction({
                    description: `Prescrição/Vendas - ${patientName}`,
                    value: totalFinal,
                    method: caixaMethod,
            ...
            <DialogContent className="max-w-[1200px] w-full bg-white dark:bg-[#0c1e3d] rounded-[24px] border-none p-0 overflow-hidden h-[90vh] flex flex-col shadow-2xl outline-none ring-0">
                ...
                <div className="px-8 py-6 bg-white dark:bg-[#0c1e3d] border-b border-gray-100 dark:border-gray-800 flex items-center justify-between shrink-0">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <DialogTitle className="text-2xl font-bold text-gray-900 dark:text-white">Editar Prescrição/Vendas</DialogTitle>
                                        ...
                                        displayItems.map((item, idx) => (
                                            <TableRow key={idx} className="hover:bg-blue-50/50 dark:hover:bg-blue-900/10 transition-colors border-b-gray-100 dark:border-b-gray-800">
                                                ...
                                                <TableCell className="text-right py-5 text-gray-600 dark:text-gray-400 font-medium">
                                                    R$ {item.displayUnitPrice.toFixed(2)}
                                                </TableCell>
                                                <TableCell className="text-right pr-6 py-5 font-bold text-[#0039A6] dark:text-blue-400 text-base">
                                                    R$ {item.displayTotal.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                                                </TableCell>
                    ...
                    <div className="col-span-12 lg:col-span-4 bg-white dark:bg-[#0c1e3d] flex flex-col h-full border-l border-gray-100 dark:border-gray-800 shadow-2xl z-10 relative overflow-hidden">
                        ...
                        <div className="flex-1 overflow-y-auto p-8 custom-scrollbar">
                            ...
                            <div className="space-y-8">
                                ...
                                {/* Status Override */}
                                <div className="space-y-3">
                                    <Label className="text-sm font-bold text-gray-700 dark:text-gray-300">Status da Prescrição/Vendas</Label>
