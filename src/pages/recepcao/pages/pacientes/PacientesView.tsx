...
import { getPacientes, removePaciente, savePacientes } from '@/services/pacientes.service'
...
import { gerarCadastroPdf } from './utils/documentoPdf'
...
export const PacientesView = ({ onPageChange }: PacientesViewProps) => {
    ...
    useEffect(() => {
        ...
        void savePacientes(patients)
    }, [patients, hasLoadedPatients])
    ...
    const handleGenerateDocumento = (patient: Patient) => {
        ...
        window.requestAnimationFrame(() => {
            ...
            void gerarCadastroPdf(documentoRef.current, patient.name)
        })
    }
        ...
        <div className="space-y-6 animate-in fade-in duration-500">
            ...
            <ExcluirPacienteModal
                ...
                onConfirm={() => {
                    ...
                    setPatients((prev) => prev.filter((item) => item.id !== selectedPaciente.id))
                    void removePaciente(selectedPaciente.id)
                                    ...
                                    <TableRow key={p.id}>
                                        <TableCell className="pl-8">
                                            <div className="py-1 flex items-center gap-3">
                                                <div className="h-9 w-9 rounded-full overflow-hidden border border-app-border dark:border-app-border-dark bg-app-bg-secondary dark:bg-white/5 shrink-0">
                                                    {p.photoUrl ? (
                                                        <img src={p.photoUrl} alt={p.name} className="h-full w-full object-cover" />
                                                    ) : (
                                                        <div className="h-full w-full flex items-center justify-center text-[10px] text-app-text-muted font-semibold">{p.name.split(' ').map((part) => part[0]).slice(0, 2).join('')}</div>
                                                    )}
                                                </div>
                                                <div className="space-y-0.5">
                                                    <p className="font-normal text-app-text-primary dark:text-white text-[14px]">{p.name}</p>
                                                    <p className="text-[12px] text-app-text-muted font-normal">{p.age} • {p.vinculoTipo || 'cliente'}</p>
                                                </div>
                                            </div>
                                        </TableCell>
                                        <TableCell>
                                            ...
                                            <div className="flex flex-col gap-1.5 py-1">
                                                ...
                                                <Badge className={`w-fit text-[10px] tracking-wider font-normal px-4 py-1 rounded-full border-none transition-all shadow-sm text-white ${
                                                    (p.activeStatus || 'Ativo') === 'Óbito'
                                                        ? 'bg-purple-800 dark:bg-purple-900 dark:text-purple-100'
                                        ...
                                        <TableCell className="text-center">
                                            <DropdownMenu>
                                                ...
                                                <DropdownMenuContent align="end" className="w-48 rounded-[12px]">
                                                    ...
                                                    <DropdownMenuItem
                                                        onClick={() => handleGenerateDocumento(p)}
                                                        className="gap-2 focus:bg-app-bg-secondary dark:focus:bg-white/5 cursor-pointer py-2.5 font-normal"
                                                    >
                                                    ...
                                                    <DropdownMenuItem
                                                        onClick={() => handleRegistrarEvolucao(p)}
                                                        className="gap-2 focus:bg-app-bg-secondary dark:focus:bg-white/5 cursor-pointer py-2.5 font-normal"
                                                    >
                                                    ...
                                                    <DropdownMenuItem
                                                        onClick={() => handleAjustePosologia(p)}
                                                        className="gap-2 focus:bg-app-bg-secondary dark:focus:bg-white/5 cursor-pointer py-2.5 font-normal"
                                                    >
                                                    ...
                                                    <DropdownMenuItem
                                                        onClick={() => handlePrescricaoComplementar(p)}
                                                        className="gap-2 focus:bg-app-bg-secondary dark:focus:bg-white/5 cursor-pointer py-2.5 font-normal"
                                                    >
            ...
            <RegistrarEvolucaoModal
                ...
                paciente={selectedPaciente ? { id: 0, nome: selectedPaciente.name } : null}
            />
            ...
            <PrescricaoComplementarModal
                ...
                paciente={selectedPaciente?.name || ''}
            />
        </div>
    )
}
