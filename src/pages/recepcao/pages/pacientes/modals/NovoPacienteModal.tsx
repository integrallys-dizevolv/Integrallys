...
import { Phone, FileText, User, MapPin, CreditCard, Heart, Upload, Camera } from 'lucide-react'
...
import { gerarCadastroPdf, abrirWhatsAppCadastro } from '../utils/documentoPdf'
import { MANUAL_SERVICOS_TEXTO_PADRAO } from '@/mocks/pacientes.mock'
...
export function NovoPacienteModal({ isOpen, onClose, onSave }: NovoPacienteModalProps) {
    ...
    const handleBuscarCnpj = async () => {
        ...
        try {
            ...
            const mockedRazao = `Fornecedor ${digits.slice(8)}`
            setFormData((prev) => ({
                ...prev,
                supplierData: {
                    ...prev.supplierData,
                    razaoSocial: prev.supplierData.razaoSocial || mockedRazao,
                    inscricaoEstadual: prev.supplierData.inscricaoEstadual || `IE-${digits.slice(0, 6)}`,
                },
            }))
        ...
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="max-w-[1000px] w-[95%] max-h-[90vh] bg-app-card dark:bg-app-card-dark p-0 overflow-hidden border-none rounded-[28px] print:hidden flex flex-col">
                <DialogHeader className="p-6 md:p-8 pb-4 border-b border-app-border dark:border-white/5 shrink-0">
                    <DialogTitle className="text-2xl font-bold text-app-text-primary dark:text-white">Novo paciente</DialogTitle>
                    <DialogDescription className="text-app-text-secondary dark:text-white/60">Preencha as informações para realizar o cadastro do cliente.</DialogDescription>
                </DialogHeader>
                    ...
                    {!isSaved ? (
                        <form id="novo-paciente-form" onSubmit={handleSubmit} className="space-y-12">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-6 gap-x-5 gap-y-5">
                                    ...
                                    <div className="md:col-span-2 space-y-2">
                                        ...
                                        <div className="relative">
                                            <DateInput id="dataNascimento" value={formData.dataNascimento} onChange={handleDateChange} className="h-11 rounded-xl border-gray-200 dark:border-white/10" required />
                                            {patientAge !== null && (
                                    ...
                                    <div className="md:col-span-2 space-y-2">
                                        <Label className="text-xs font-bold text-[#344054] dark:text-gray-300 uppercase tracking-tight">Tipo de vínculo</Label>
                                    ...
                                    <div className="md:col-span-2 space-y-2">
                                        <Label className="text-xs font-bold text-[#344054] dark:text-gray-300 uppercase tracking-tight">Foto do paciente</Label>
                                        <div className="flex items-center gap-3 rounded-xl border border-gray-200 dark:border-white/10 p-3">
                                            <label className="h-14 w-14 rounded-full overflow-hidden border border-app-border dark:border-white/10 bg-app-bg-secondary dark:bg-white/5 shrink-0 cursor-pointer relative block">
                                                {formData.photoUrl ? (
                                                    <img src={formData.photoUrl} alt="Prévia do paciente" className="h-full w-full object-cover" />
                                                ) : (
                                            ...
                                            <label className="inline-flex items-center gap-2 h-10 px-3 rounded-lg border border-app-border dark:border-white/10 cursor-pointer hover:bg-app-bg-secondary dark:hover:bg-white/5 text-xs font-bold uppercase tracking-wide">
                            ...
                            {(formData.vinculoTipo === 'fornecedor' || formData.vinculoTipo === 'prestador') && (
                                <div className="mt-6 p-4 rounded-xl border border-app-border dark:border-white/10 bg-app-bg-secondary/40 dark:bg-white/5">
                                    <h4 className="text-sm font-bold text-[#101828] dark:text-white uppercase tracking-widest mb-4">
                                        Dados de fornecedor/prestador
                                    </h4>
                                    <div className="grid grid-cols-1 md:grid-cols-6 gap-x-5 gap-y-4">
                                        <div className="md:col-span-3 space-y-2">
                                            <Label className="text-xs font-bold text-[#344054] dark:text-gray-300 uppercase tracking-tight">Razão social</Label>
                                            <Input value={formData.supplierData.razaoSocial} onChange={(e) => setFormData({ ...formData, supplierData: { ...formData.supplierData, razaoSocial: e.target.value } })} className="h-11 rounded-xl border-gray-200 dark:border-white/10" />
                                        </div>
                                        <div className="md:col-span-2 space-y-2">
                                            <Label className="text-xs font-bold text-[#344054] dark:text-gray-300 uppercase tracking-tight">CNPJ</Label>
                                            <Input value={formData.supplierData.cnpj} onChange={(e) => setFormData({ ...formData, supplierData: { ...formData.supplierData, cnpj: e.target.value } })} className="h-11 rounded-xl border-gray-200 dark:border-white/10" placeholder="00.000.000/0000-00" />
                                        </div>
                                        ...
                                        <div className="md:col-span-2 space-y-2">
                                            <Label className="text-xs font-bold text-[#344054] dark:text-gray-300 uppercase tracking-tight">Inscrição estadual</Label>
                                            <Input value={formData.supplierData.inscricaoEstadual} onChange={(e) => setFormData({ ...formData, supplierData: { ...formData.supplierData, inscricaoEstadual: e.target.value } })} className="h-11 rounded-xl border-gray-200 dark:border-white/10" />
                                        </div>
                                        <div className="md:col-span-2 space-y-2">
                                            <Label className="text-xs font-bold text-[#344054] dark:text-gray-300 uppercase tracking-tight">Contato</Label>
                                            <Input value={formData.supplierData.contatoNome} onChange={(e) => setFormData({ ...formData, supplierData: { ...formData.supplierData, contatoNome: e.target.value } })} className="h-11 rounded-xl border-gray-200 dark:border-white/10" />
                                        </div>
                                        <div className="md:col-span-1 space-y-2">
                                            <Label className="text-xs font-bold text-[#344054] dark:text-gray-300 uppercase tracking-tight">Setor</Label>
                                            <Input value={formData.supplierData.contatoSetor} onChange={(e) => setFormData({ ...formData, supplierData: { ...formData.supplierData, contatoSetor: e.target.value } })} className="h-11 rounded-xl border-gray-200 dark:border-white/10" placeholder="Compras" />
                                        </div>
                                        <div className="md:col-span-1 space-y-2">
                                            <Label className="text-xs font-bold text-[#344054] dark:text-gray-300 uppercase tracking-tight">Categoria DRE</Label>
                                            <Input value={formData.supplierData.categoriaDre} onChange={(e) => setFormData({ ...formData, supplierData: { ...formData.supplierData, categoriaDre: e.target.value } })} className="h-11 rounded-xl border-gray-200 dark:border-white/10" />
                                        </div>
                                    </div>
                                </div>
                            )}
                            ...
                            <div className="bg-white dark:bg-white/5 p-6 rounded-2xl border border-app-border dark:border-white/5 shadow-sm animate-in fade-in slide-in-from-top-2">
                                ...
                                {(needsMandatoryResponsible || showOptionalResponsible || formData.responsible.name) && (
                                    <div className="mt-8 p-6 bg-[#F8FAFC] dark:bg-white/5 rounded-2xl border border-gray-100 dark:border-white/5 space-y-6">
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            ...
                                            <div className="space-y-2">
                                                ...
                                                <DateInput id="resp-nasc" value={formData.responsible.birthDate} onChange={handleRespDateChange} className="h-11 rounded-xl bg-white dark:bg-transparent border-gray-200 dark:border-white/10" />
                                            </div>
                                            <div className="space-y-2">
                                            ...
                                            <div className="space-y-4">
                                                ...
                                                <div className="flex flex-col sm:flex-row gap-6">
                                                    <label className="flex items-center gap-3 cursor-pointer group">
                                                        ...
                                                        <span className="text-sm font-medium dark:text-gray-300">Mesmos dados do cadastro</span>
                                                    </label>
                                                    <label className="flex items-center gap-3 cursor-pointer group">
                                                        ...
                                                        <span className="text-sm font-medium dark:text-gray-300">Outros dados</span>
                                                    </label>
                                                </div>
                                            </div>
                        ...
                        <div className="py-12 flex flex-col items-center text-center space-y-8 animate-in zoom-in-95 duration-500">
                            ...
                            <div className="space-y-3">
                                <h3 className="text-3xl font-black text-[#101828] dark:text-white uppercase tracking-tighter">Paciente Cadastrado!</h3>
                                <p className="text-[#667085] dark:text-gray-400 text-base max-w-sm mx-auto">O registro de <strong>{formData.nome}</strong> foi finalizado com sucesso.</p>
                            </div>
                            ...
                            <div className="w-full max-w-lg pt-6 grid grid-cols-1 gap-3">
                                ...
                                <Button variant="outline" className="h-12 rounded-xl border-[#0039A6] text-[#0039A6]" onClick={() => void handleSaveAndGeneratePdf()}>
                                    Salvar e Gerar PDF
                                </Button>
                                <Button className="h-12 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white" onClick={() => void handleSaveAndWhatsapp()}>
                ...
                <DialogFooter className="p-6 md:p-8 pt-6 border-t border-app-border dark:border-white/5 bg-app-bg-secondary dark:bg-app-card-dark/50 flex justify-between items-center shrink-0">
                    {!isSaved ? (
                        <div className="flex flex-col-reverse sm:flex-row gap-3 w-full">
                            ...
                            <Button form="novo-paciente-form" type="submit" className="flex-1 h-12 bg-[#0039A6] hover:bg-[#002d82] text-white font-black rounded-xl shadow-xl shadow-[#0039A6]/20 transition-all active:scale-[0.98] text-xs uppercase tracking-[0.1em]">
                                Salvar Cadastro
                            </Button>
                        </div>
                    ) : (
                ...
                <div className="hidden print:block p-12 space-y-10 text-black bg-white">
                    <div className="flex items-center justify-between border-b-4 border-black pb-8">
                        <div>
                            <h1 className="text-4xl font-black italic tracking-tighter">INTEGRALLYS</h1>
                            <p className="text-[10px] font-black uppercase tracking-[0.3em] mt-1">Centro de Desenvolvimento Humano</p>
                        </div>
                        <div className="text-right">
                            <h2 className="text-lg font-black uppercase underline">Ficha Cadastral</h2>
                            <p className="text-[10px] font-bold mt-1">DATA DE EMISSÃO: {new Date().toLocaleDateString('pt-BR')}</p>
                        </div>
                    </div>

                    <div className="grid grid-cols-2 gap-12 py-6">
                        <div className="space-y-3 text-xs">
                            <h3 className="font-black border-b-2 border-black text-[9px] uppercase mb-4 tracking-widest py-1 bg-gray-100 px-2">Identificação do Paciente</h3>
                        ...
                        <div className="space-y-3 text-xs">
                            <h3 className="font-black border-b-2 border-black text-[9px] uppercase mb-4 tracking-widest py-1 bg-gray-100 px-2">Contato e Residência</h3>
                    ...
                    {formData.responsible.name && (
                        <div className="p-6 border-2 border-black bg-gray-50 rounded-lg">
                            <h3 className="font-black text-[9px] uppercase mb-4 tracking-widest">Responsável Legal do Paciente</h3>
                    ...
                    <div className="space-y-6 pt-10">
                        ...
                        <div className="text-[10px] space-y-4 leading-relaxed text-justify px-4">
                            ...
                            <p><strong>2. POLÍTICA DE ATENDIMENTO:</strong> O paciente e seus responsáveis estão cientes das normas internas da Integrallys, incluindo a necessidade de aviso prévio de 24 horas para cancelamento de sessões e as consequências de faltas injustificadas.</p>
                            <p><strong>3. PROTEÇÃO DE DADOS:</strong> Em conformidade com a LGPD (Lei 13.709/18), a Integrallys compromete-se a utilizar os dados coletados exclusivamente para fins de atendimento clínico e administrativo, garantindo o sigilo absoluto das informações.</p>
                        </div>
                    </div>

                    <div className="pt-32 flex justify-between gap-20">
                        <div className="flex-1 border-t-2 border-black pt-3 text-center">
                            <p className="text-[10px] font-black uppercase tracking-tighter">{formData.nome.toUpperCase()}</p>
                            <p className="text-[8px] font-bold text-gray-600">ASSINATURA DO PACIENTE OU RESPONSÁVEL</p>
                        </div>
                        <div className="flex-1 border-t-2 border-black pt-3 text-center">
                            <p className="text-[10px] font-black uppercase tracking-tighter">INTEGRALLYS CDH</p>
                            <p className="text-[8px] font-bold text-gray-600">CARIMBO E VISTO ADM / RECEPÇÃO</p>
                        </div>
                    </div>
                </div>
            </DialogContent>
