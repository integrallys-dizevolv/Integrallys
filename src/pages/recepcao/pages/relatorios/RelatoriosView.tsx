...
import { MOCK_RELATORIOS_RECEPCAO_FILTROS } from '@/mocks/relatorios.mock'
...
export const RelatoriosView = ({ onPageChange }: RelatoriosViewProps) => {
    ...
    const [customSales, setCustomSales] = useState<SaleReportItem[]>([])
    ...
    const [cancellationClientFilter, setCancellationClientFilter] = useState('todos')
    const [cancellationProcedureFilter, setCancellationProcedureFilter] = useState('todos')
    ...
    useEffect(() => {
        const loadCustomSales = () => {
            try {
                const raw = localStorage.getItem('recepcao_relatorio_vendas_custom')
                const parsed: SaleReportItem[] = raw ? JSON.parse(raw) : []
                setCustomSales(Array.isArray(parsed) ? parsed : [])
            } catch {
                setCustomSales([])
            }
        }
        ...
        loadCustomSales()
        window.addEventListener('storage', loadCustomSales)
        return () => window.removeEventListener('storage', loadCustomSales)
    }, [])


    const tabs = [
        { name: 'Consultas', icon: Calendar },
        { name: 'Justificativas de Cancelamento', icon: XCircle },
        { name: 'Retornos', icon: RotateCcw },
        { name: 'Recebimentos de Cartão', icon: DollarSign },
        { name: 'Prescrição/Vendas', icon: ShoppingBag },
    ...
    const filteredConsultas = useMemo(() => {
        return MOCK_RELATORIO_CONSULTAS.filter(item => {
            ...
            const matchesStatus =
                statusFilter === 'todos' ||
                (statusFilter === 'Disponiveis' && item.statusConsulta === 'Disponivel') ||
                (statusFilter === 'Nao Compareceu' && item.statusConsulta === 'Nao Compareceu') ||
                (statusFilter === 'Realizados' && item.statusConsulta === 'Realizado') ||
                item.statusConsulta === statusFilter
    ...
    const filteredPacientes = useMemo(() => {
        return MOCK_PACIENTES_RELATORIO.filter((patient: DashboardPatient) => {
            ...
            // Source (Mídia)
            const normalizedSource = patient.source === 'Google Search' ? 'Outro' : patient.source
            const matchesSource = sourceFilter === 'todos' || normalizedSource === sourceFilter
    ...
    const filteredVendas = useMemo(() => {
        return [...customSales, ...MOCK_RELATORIO_VENDAS].filter((item: SaleReportItem) => {
            // Search (Patient Name)
    ...
    }, [customSales, salesSearchTerm, isWithinPeriod, salesProductFilter, salesProfessionalFilter, salesUserFilter, salesUnitFilter, salesPaymentMethodFilter])

    const salesProductOptions = useMemo(() => {
        const dynamic = new Set([...customSales, ...MOCK_RELATORIO_VENDAS].map((item) => item.productName))
        return Array.from(new Set([...MOCK_RELATORIOS_RECEPCAO_FILTROS.produtos, ...dynamic]))
    }, [customSales])

    const salesUserOptions = useMemo(() => {
        const dynamic = new Set([...customSales, ...MOCK_RELATORIO_VENDAS].map((item) => item.userName))
        return Array.from(new Set([...MOCK_RELATORIOS_RECEPCAO_FILTROS.usuarios, ...dynamic]))
    }, [customSales])
    ...
    // Filtered Budgets
    const getBudgetStatusMeta = (item: BudgetReportItem) => {
        if (item.status === 'Convertido') {
            return { statusType: 'Convertido' as const, statusLabel: 'Convertido' }
        }

        const dayMs = 1000 * 60 * 60 * 24
        const now = new Date()
        const today = new Date(now.getFullYear(), now.getMonth(), now.getDate())
        const creation = new Date(`${item.creationDate}T00:00:00`)
        const expiration = new Date(`${item.expirationDate}T00:00:00`)

        const totalDays = Math.max(1, Math.floor((expiration.getTime() - creation.getTime()) / dayMs) + 1)
        const remainingDays = Math.max(0, Math.floor((expiration.getTime() - today.getTime()) / dayMs) + 1)

        if (remainingDays > 0) {
            return {
                statusType: 'No Prazo' as const,
                statusLabel: `No Prazo - ${remainingDays}/${totalDays}`,
            }
        }

        return { statusType: 'Vencido' as const, statusLabel: 'Vencido' }
    }
    ...
    const filteredBudgets = useMemo(() => {
        return MOCK_RELATORIO_ORCAMENTOS.filter((item: BudgetReportItem) => {
            ...
            const statusMeta = getBudgetStatusMeta(item)
            const matchesStatus = budgetStatusFilter === 'todos' || statusMeta.statusType === budgetStatusFilter
    ...
    const budgetTotals = useMemo(() => {
        return filteredBudgets.reduce((acc, curr) => {
            const statusMeta = getBudgetStatusMeta(curr)
            acc.totalValue += curr.totalValue
            if (statusMeta.statusType === 'No Prazo') acc.totalWithinDeadline += curr.totalValue
            if (statusMeta.statusType === 'Vencido') acc.totalExpired += curr.totalValue
            return acc
    ...
    const cancelamentoClientOptions = useMemo(() => {
        const unique = new Set(cancelamentosFonte.map((item) => item.paciente))
        return Array.from(unique)
    }, [cancelamentosFonte])

    const cancelamentoProcedureOptions = useMemo(() => {
        const unique = new Set(cancelamentosFonte.map((item) => item.procedimento).filter((value): value is string => Boolean(value)))
        return Array.from(unique)
    }, [cancelamentosFonte])
    ...
    const filteredCancelamentos = useMemo(() => {
        return cancelamentosFonte.filter((item: RelatorioCancelamentoItem) => {
            ...
            const matchesClient = cancellationClientFilter === 'todos' || item.paciente === cancellationClientFilter
            const matchesProcedure = cancellationProcedureFilter === 'todos' || item.procedimento === cancellationProcedureFilter

            return matchesSearch && matchesPeriod && matchesStatus && matchesProfessional && matchesClient && matchesProcedure
        })
    }, [cancellationSearchTerm, isWithinPeriod, cancellationStatusFilter, cancellationProfessionalFilter, cancellationClientFilter, cancellationProcedureFilter, cancelamentosFonte])
    ...
    const getReceiptFees = (item: RelatorioRecebimentoItem) => {
        const taxaPercent = 2.99
        const valorTaxaTotal = item.valorBruto * (taxaPercent / 100)
    ...
    const filteredReceipts = useMemo(() => {
        return MOCK_RELATORIO_RECEBIMENTOS.filter((item: RelatorioRecebimentoItem) => {
            ...
            const isCard = item.formaPagamento === 'Cartão de Crédito' || item.formaPagamento === 'Cartão de Débito'

            return matchesSearch && matchesPeriod && matchesStatus && matchesPayment && isCard
        })
    ...
    const mergedRetornos = useMemo<RelatorioRetornoItem[]>(() => {
        const customRetornos = loadRetornosFollowup().map((item, index) => ({
            id: Number(String(Date.now() + index).slice(-9)),
    ...
    }, [period, retornosRefreshTick])

    const retornoProfessionalOptions = useMemo(() => {
        const unique = new Set(mergedRetornos.map(item => item.profissional))
        return Array.from(unique)
    }, [mergedRetornos])

    const retornoSpecialtyOptions = useMemo(() => {
        const unique = new Set(mergedRetornos.map(item => item.especialidade))
        return Array.from(unique)
    }, [mergedRetornos])

    const filteredRetornos = useMemo(() => {
        return mergedRetornos.filter((item: RelatorioRetornoItem) => {
    ...
    }, [returnsSearchTerm, returnsProfessionalFilter, returnsSpecialtyFilter, returnsStatusFilter, isWithinPeriod, mergedRetornos])
        ...
        <div className="space-y-8 animate-in fade-in duration-500 max-w-[1600px] mx-auto pb-10">
            ...
            <div className="bg-app-card dark:bg-app-card-dark rounded-[24px] border border-app-border dark:border-app-border-dark shadow-sm min-h-[600px] overflow-hidden flex flex-col">
                <div className="p-8 flex-1 flex flex-col space-y-8">
                            ...
                            <div className="w-10 h-10 rounded-xl bg-app-bg-secondary dark:bg-white/5 flex items-center justify-center">
                                {activeTab === 'Consultas' ? <Calendar className="text-[#101828] dark:text-white" size={20} /> :
                                    activeTab === 'Pacientes' ? <Users className="text-[#101828] dark:text-white" size={20} /> :
                                        activeTab === 'Prescrição/Vendas' ? <ShoppingBag className="text-[#101828] dark:text-white" size={20} /> :
                                            activeTab === 'Justificativas de Cancelamento' ? <XCircle className="text-[#101828] dark:text-white" size={20} /> :
                                                activeTab === 'Recebimentos de Cartão' ? <DollarSign className="text-[#101828] dark:text-white" size={20} /> :
                        ...
                        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-500 flex-1 flex flex-col">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-3">
                                ...
                                <Select value={statusFilter} onValueChange={setStatusFilter}>
                                    ...
                                    <SelectContent>
                                        ...
                                        <SelectItem value="Nao Compareceu">Não comparecimentos</SelectItem>
                                    </SelectContent>
                                </Select>
                            </div>
                                ...
                                <Table>
                                    <TableHeader className="bg-app-bg-secondary/50 dark:bg-white/5">
                                        <TableRow className="hover:bg-transparent border-b border-app-border dark:border-app-border-dark">
                                            ...
                                            <TableHead className="font-normal text-xs uppercase text-app-text-secondary">Forma de Pagamento</TableHead>
                                                    ...
                                                    <TableCell className="py-4">
                                                        <div className="flex flex-col gap-1">
                                                            <Badge variant="outline" className={cn("w-fit font-normal text-[10px] border-none px-2",
                                                                ...
                                                                {item.statusPagamento === 'Pago' ? 'Pago' : 'Em aberto'}
                                                            </Badge>
                                                        </div>
                                                    </TableCell>
                                                    <TableCell className="py-4">
                                                        <span className="text-xs text-app-text-muted px-1">
                                                            {item.metodoPagamento && item.metodoPagamento.length > 0 ? item.metodoPagamento.join(', ') : '-'}
                                                        </span>
                                                    </TableCell>
                                                    <TableCell className="py-4">
                                                        <div className="flex flex-col">
                            ...
                            <div className="flex flex-col md:flex-row md:items-center justify-end gap-6 px-2">
                                <p className="text-sm text-emerald-600 dark:text-emerald-400">
                                    Total Recebido: R$ {totals.received.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                                </p>
                                <p className="text-sm text-red-600 dark:text-red-400">
                                    Total em Aberto: R$ {totals.open.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                                </p>
                            </div>
                    ...
                    {activeTab === 'Justificativas de Cancelamento' && (
                        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-500 flex-1 flex flex-col">
                            {/* Header / Totalizer */}
                            <div className="bg-app-bg-secondary/30 dark:bg-white/5 rounded-xl p-6 border border-app-border dark:border-app-border-dark">
                                <div className="flex flex-col md:flex-row md:items-center justify-end gap-8">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-3">
                                ...
                                <Select value={cancellationClientFilter} onValueChange={setCancellationClientFilter}>
                                    <SelectTrigger className="h-10 rounded-xl border-app-border bg-app-bg-secondary/30">
                                        <SelectValue preferPlaceholder placeholder="Todos os Clientes" />
                                    </SelectTrigger>
                                    <SelectContent>
                                        <SelectItem value="todos">Todos os Clientes</SelectItem>
                                        {cancelamentoClientOptions.map((cliente) => (
                                            <SelectItem key={cliente} value={cliente}>{cliente}</SelectItem>
                                        ))}
                                    </SelectContent>
                                </Select>
                                <Select value={cancellationProcedureFilter} onValueChange={setCancellationProcedureFilter}>
                                    <SelectTrigger className="h-10 rounded-xl border-app-border bg-app-bg-secondary/30">
                                        <SelectValue preferPlaceholder placeholder="Todos os Procedimentos" />
                                    </SelectTrigger>
                                    <SelectContent>
                                        <SelectItem value="todos">Todos os Procedimentos</SelectItem>
                                        {cancelamentoProcedureOptions.map((procedimento) => (
                                ...
                                <Table>
                                    <TableHeader className="bg-app-bg-secondary/50 dark:bg-white/5">
                                        <TableRow className="hover:bg-transparent border-b border-app-border dark:border-app-border-dark">
                                            ...
                                            <TableHead className="h-11 text-xs font-medium text-app-text-secondary uppercase tracking-wider">Procedimento</TableHead>
                                        ...
                                        {filteredCancelamentos.length > 0 ? (
                                            filteredCancelamentos.map((item) => (
                                                <TableRow key={item.id} className="hover:bg-app-bg-secondary/30 dark:hover:bg-white/5 transition-colors border-b border-app-border/50">
                                                    ...
                                                    <TableCell className="py-4">
                                                        <span className="text-xs text-app-text-secondary dark:text-white/70">{item.procedimento || '-'}</span>
                                                    </TableCell>
                                                    <TableCell className="py-4">
                                                        <div className="flex flex-col">
                                                            <span className="text-app-text-primary dark:text-white text-sm">{item.profissional}</span>
                        ...
                        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-500 flex-1 flex flex-col">
                            ...
                            <div className="rounded-xl border border-app-border dark:border-app-border-dark overflow-hidden flex-1">
                                <Table>
                                    <TableHeader className="bg-app-bg-secondary/50 dark:bg-white/5">
                                        <TableRow className="hover:bg-transparent border-b border-app-border dark:border-app-border-dark">
                                            <TableHead className="h-11 text-xs font-medium text-app-text-secondary uppercase tracking-wider pl-6">Data atendimento</TableHead>
                                            <TableHead className="h-11 text-xs font-medium text-app-text-secondary uppercase tracking-wider">Nome paciente</TableHead>
                                            <TableHead className="h-11 text-xs font-medium text-app-text-secondary uppercase tracking-wider">Últimos 3 procedimentos</TableHead>
                                            <TableHead className="h-11 text-xs font-medium text-app-text-secondary uppercase tracking-wider">Especialista</TableHead>
                                            <TableHead className="h-11 text-xs font-medium text-app-text-secondary uppercase tracking-wider">Data prevista retorno</TableHead>
                    ...
                    {activeTab === 'Recebimentos de Cartão' && (
                        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-500 flex-1 flex flex-col">
                            {/* Header / Totalizer */}
                            <div className="bg-app-bg-secondary/30 dark:bg-white/5 rounded-xl p-6 border border-app-border dark:border-app-border-dark">
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                            ...
                            <div className="rounded-xl border border-app-border dark:border-app-border-dark overflow-hidden flex-1">
                                <Table>
                                    <TableHeader className="bg-app-bg-secondary/50 dark:bg-white/5">
                                        <TableRow className="hover:bg-transparent border-b border-app-border dark:border-app-border-dark">
                                            <TableHead className="h-11 text-xs font-medium text-app-text-secondary uppercase tracking-wider pl-6">Data de compensação</TableHead>
                                            <TableHead className="h-11 text-xs font-medium text-app-text-secondary uppercase tracking-wider">Cliente</TableHead>
                                            <TableHead className="h-11 text-xs font-medium text-app-text-secondary uppercase tracking-wider">Taxa descontada</TableHead>
                                            ...
                                            <TableHead className="h-11 text-xs font-medium text-app-text-secondary uppercase tracking-wider text-right">Descontos</TableHead>
                                        ...
                                        {filteredReceipts.length > 0 ? (
                                            filteredReceipts.map((item) => {
                                                ...
                                                <TableRow key={item.id} className="hover:bg-app-bg-secondary/30 dark:hover:bg-white/5 transition-colors border-b border-app-border/50">
                                                    ...
                                                    <TableCell className="py-4">
                                                        <span className="text-sm text-app-text-primary dark:text-white">{receiptFees.taxaPercent.toFixed(2)}%</span>
                                                    </TableCell>
                                                    <TableCell className="py-4 text-right">
                            ...
                            <div className="bg-app-bg-secondary/30 dark:bg-white/5 rounded-xl p-6 border border-app-border dark:border-app-border-dark">
                                <div className="flex flex-col md:flex-row md:items-center justify-end gap-8">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className="flex flex-col">
                                            <span className="text-xs text-app-text-secondary uppercase tracking-wider">Total de Prescrição/Vendas</span>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-3">
                                ...
                                <Select value={salesProductFilter} onValueChange={setSalesProductFilter}>
                                    ...
                                    <SelectContent>
                                        ...
                                        {salesProductOptions.map((produto) => (
                                                ...
                                                <TableRow key={item.id} className="hover:bg-app-bg-secondary/30 dark:hover:bg-white/5 transition-colors border-b border-app-border/50">
                                                    ...
                                                    <TableCell className="py-4">
                                                        <Badge variant="outline" className={cn("font-normal text-xs border-none px-2",
                                                            statusMeta.statusType === 'No Prazo' ? "bg-emerald-50 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-400" :
                                                                statusMeta.statusType === 'Convertido' ? "bg-blue-50 text-blue-700 dark:bg-blue-500/20 dark:text-blue-300" :
                                                                    "bg-red-50 text-red-700 dark:bg-red-500/20 dark:text-red-400"
                                                        )}>
                            ...
                            <div className="rounded-xl border border-app-border dark:border-app-border-dark overflow-hidden flex-1">
                                <Table>
                                    <TableHeader className="bg-app-bg-secondary/50 dark:bg-white/5">
                                        <TableRow className="hover:bg-transparent border-b border-app-border dark:border-app-border-dark">
                                            <TableHead className="font-normal text-xs uppercase text-app-text-secondary">Paciente</TableHead>
                                            <TableHead className="font-normal text-xs uppercase text-app-text-secondary">Mídia de Captação</TableHead>
                                            <TableHead className="font-normal text-xs uppercase text-app-text-secondary">Cidade</TableHead>
