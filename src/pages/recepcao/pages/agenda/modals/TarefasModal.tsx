...
interface Tarefa extends TarefaItem {}
...
export function TarefasModal({
  ...
  useEffect(() => {
    const sync = () => {
      const stored = loadTarefas();
      if (stored.length === 0) {
        const seeded = MOCK_TAREFAS.map((item) => ({
          ...item,
          perfil,
          usuarioCriador,
        }));
      ...
      if (perfil === "gestor" || perfil === "admin") {
        setTarefas(stored as Tarefa[]);
      } else {
        setTarefas(
          stored.filter(
            (item) => item.perfil === perfil && item.usuarioCriador === usuarioCriador,
          ) as Tarefa[],
        );
      }
    };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (tarefaEditando) {
      ...
      upsertTarefa(updated);
      setTarefas(tarefas.map((t) => (t.id === tarefaEditando.id ? updated : t)));
    } else {
  ...
  const handleToggleStatus = (id: string) => {
    const next: Tarefa[] = tarefas.map((t) =>
      t.id === id
        ? {
            ...t,
            status: (t.status === "Pendente" ? "Concluída" : "Pendente") as
              | "Pendente"
              | "Concluída",
          }
        : t,
    ...
    if (changed) upsertTarefa(changed);
    setTarefas(next);
  };
  ...
  const hierarchyLabel = useMemo(() => {
    if (perfil === "admin") return "Hierarquia: Administrador"
    if (perfil === "gestor") return "Hierarquia: Gestor"
    return "Hierarquia: Individual"
  }, [perfil])
