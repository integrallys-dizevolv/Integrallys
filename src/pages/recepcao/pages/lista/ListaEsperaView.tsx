...
} from '@/services/listaEspera.service'
...
export const ListaEsperaView = ({ onPageChange }: ListaEsperaViewProps) => {
    ...
    const [patients, setPatients] = useState(() => loadWaitingList())

    useEffect(() => {
        const syncFromStorage = () => setPatients(loadWaitingList())
    ...
    const filteredPatients = useMemo(() => {
        return patients.filter(p => {
            ...
            const matchesPreferencia = preferenciaFilter === 'todos' ||
                (p.preferenciaHorario || '').toLowerCase() === preferenciaFilter.toLowerCase();
            ...
            <div className="flex flex-col lg:flex-row justify-between items-center gap-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-4 gap-3 w-full lg:w-auto">
                    ...
                    <Select value={preferenciaFilter} onValueChange={setPreferenciaFilter}>
                        <SelectTrigger className="w-full lg:w-[170px] h-11 text-xs px-4 rounded-xl border-gray-200 dark:border-gray-800 bg-white dark:bg-[#0c1e3d] focus:ring-[#0039A6] focus:border-[#0039A6]">
                                        ...
                                        <DropdownMenu>
                                            ...
                                            <DropdownMenuContent align="end" className="w-40 p-1 rounded-xl shadow-xl border-app-border dark:border-app-border-dark dark:bg-app-bg-dark">
                                                ...
                                                <DropdownMenuItem
                                                    ...
                                                    onClick={() => {
                                                        setPatients(removeWaitingPatient(patient.id))
                                                        toast.success(`${patient.name} removido(a) da lista de espera.`)
                                                    }}
                                                >
                                                    <Trash2 size={16} />
