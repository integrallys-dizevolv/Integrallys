...
import type { User, Permission, Unit } from '../../../components/types'
...
export function GerenciamentoPerfis({ onPageChange }: GerenciamentoPerfisProps) {
  ...
  const permissionColumns = [
    { header: 'Perfil', render: (p: any) => <span className="font-normal text-app-text-primary dark:text-white">{p.nome}</span> },
    {
      header: 'Descrição',
      render: (p: any) => <span className="text-[#1A1A1AB2] dark:text-[#c3cec9] max-w-xs block truncate">{p.descricao}</span>
    },
    {
      header: 'Escopo',
      render: (p: any) => (
