...
import { registrarBaixaAutomaticaSuprimentos } from "@/services/suprimentos.service";
import {
  registrarCancelamento,
  registrarGeracaoAgenda,
...
const CANCELAMENTO_MIN_REASON = 10;
...
export function AgendaView({
  ...
  const normalizeStatus = (status: string) => {
    ...
    if (status === "Em atraso") return "Em Atraso";
    return status;
  };
  ...
  const getBlockInfo = (date: Date, time: string, professionalName: string) => {
    const targetMinutes = parseTimeToMinutes(time);

    const block = agendaBlocks.find((item) => {
      if (item.profissional !== professionalName) return false;
      if (!isDateWithinRange(date, item.dataInicio, item.dataFim)) return false;
      if (item.bloquearDiaInteiro) return true;

      const inicio = parseTimeToMinutes(item.horarioInicio);
      const fim = parseTimeToMinutes(item.horarioFim);
      return targetMinutes >= inicio && targetMinutes < fim;
    });
    ...
    return {
      motivo: block.justificativa,
      tipo: block.bloquearDiaInteiro ? "Dia inteiro" : "Horário específico",
    };
  };
  ...
  const [bloqueioInitialData, setBloqueioInitialData] = useState<
    { data?: string; horario?: string; profissional?: string } | undefined
  >(undefined);
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case "Check-out":
        return "bg-emerald-800/15 dark:bg-emerald-900/45";
      case "Atraso":
      case "Em Atraso":
        return "bg-orange-100 dark:bg-orange-900/25";
      case "Em Atendimento":
        return "bg-purple-100 dark:bg-purple-900/25 border-l-4 border-purple-500";
      case "Cancelado":
        return "bg-gray-300/80 dark:bg-gray-800/80 opacity-85";
      default:
  ...
  const isPagamentoTotal = (item: AgendaItem) => {
    if (item.pagamento === "Pago") return true;
    if (!item.pagamentos?.length || !item.valorProcedimento) return false;

    const totalPago = item.pagamentos.reduce(
      (acc, pagamento) => acc + pagamento.valor,
      0,
    );

    return totalPago >= item.valorProcedimento;
  };

  const handleEmitirReciboOuNf = (item: AgendaItem) => {
    setSelectedConsulta(item);
    toast.success("Emissão de recibo ou NF disponível para este atendimento.");
  };
  ...
  const handleConfirmCancelamento = () => {
    ...
    if (!motivo || cancelamentoItemId === null) {
      setCancelamentoErro("Informe o motivo do cancelamento");
      return;
    }

    if (motivo.length < CANCELAMENTO_MIN_REASON) {
      setCancelamentoErro(
        `Informe pelo menos ${CANCELAMENTO_MIN_REASON} caracteres para o motivo`,
      );
      return;
    }
    ...
    if (cancelledItem) {
      void registrarCancelamento(cancelamentoItemId, motivo);
  ...
  const handleConfirmCobranca = (payload: {
    ...
    const totalPagoAnterior = pagamentosAtuais.reduce((acc, p) => acc + p.valor, 0);
    ...
    const virouPagoTotal =
      valorTotal > 0 && totalPago >= valorTotal && totalPagoAnterior < valorTotal;

    if (virouPagoTotal) {
      registrarBaixaAutomaticaSuprimentos({
        procedimento: selectedConsulta.tipo || "Consulta",
        paciente: selectedConsulta.paciente,
        usuario: "Recepção",
      });
      toast.success("Suprimentos baixados automaticamente.");
    }
  };
  ...
  const handleConfirmCancelarConsulta = (reason: string) => {
    ...
    void registrarCancelamento(itemId, reason.trim());
  ...
  const handleEditarCompromisso = (
    id: number,
    data: Partial<AgendaPersonalItem> & { horario?: string },
  ) => {
    void id;
    void data;
  };
  ...
  const handleCancelarCompromisso = (id: number) => {
    void id;
  };
  ...
  const handleBloquearAgenda = (payload: {
    ...
    setBloqueioInitialData(undefined);
  };

  const handleOpenBloqueioFromSlot = (
    date: Date,
    time: string,
    professional: string,
  ) => {
    setBloqueioInitialData({
      data: format(date, "yyyy-MM-dd"),
      horario: time,
      profissional: professional,
    });
    setIsBloqueioAgendaOpen(true);
  };
  ...
  return (
    <div className="space-y-6 overflow-x-hidden max-w-full">
      ...
      <BloqueioAgendaModal
        ...
        initialData={bloqueioInitialData}
        ...
        <DialogContent className="max-w-[520px] bg-app-card dark:bg-app-card-dark">
          ...
          <div className="space-y-2">
            <Label className="text-[13px] font-normal text-gray-700 dark:text-white/80">
              Informe o motivo para gerar o relatório de justificativas (mínimo de 10 caracteres)
            </Label>
            <Textarea
          ...
          <DialogFooter className="gap-3 sm:gap-0">
            ...
            <Button
              ...
              disabled={cancelamentoMotivo.trim().length < CANCELAMENTO_MIN_REASON}
      ...
      {!hideGlobalTab && (
        <SegmentedControl
          options={[
            { value: "global", label: "Somente Atendimentos" },
            ...(allowPersonalTab
              ? [{ value: "pessoal", label: "Somente Agenda Pessoal" }]
              : []),
            ...(allowMixedView
              ? [{ value: "misto", label: "Ambas" }]
              : []),
          ]}
          value={activeTab}
                      ...
                      return (
                        <div key={slot.id} className="space-y-2">
                          ...
                          {targetNames.map((proName) => {
                            ...
                            if (availability.blocked && blockInfo) {
                              return (
                                <div
                                  key={`blocked-${slot.id}-${proName}`}
                                  title={`Bloqueado (${blockInfo.tipo}): ${blockInfo.motivo}`}
                                  className="w-full border border-gray-300 dark:border-gray-700 rounded-[10px] p-4 flex items-center gap-4 h-[88px]"
                                  style={{
                                    backgroundImage:
                                      "repeating-linear-gradient(135deg, rgba(148,163,184,.25), rgba(148,163,184,.25) 8px, rgba(148,163,184,.1) 8px, rgba(148,163,184,.1) 16px)",
                                  }}
                                >
                                  <div className="w-[60px] text-left">
                                    <p className="text-[20px] font-normal text-gray-500 leading-[30px]">
                                      {slot.time}
                                    </p>
                                  </div>
                            ...
                            return (
                              <button
                                ...
                                onContextMenu={(event) => {
                                  ...
                                  handleOpenBloqueioFromSlot(currentDate, slot.time, proName);
                                }}
                      ...
                      return (
                        <button
                          ...
                          onContextMenu={(event) => {
                            ...
                            handleOpenBloqueioFromSlot(dayDate, "08:00", targetProfessional);
                          }}
