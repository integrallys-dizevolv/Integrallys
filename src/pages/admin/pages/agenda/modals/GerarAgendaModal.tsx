    ...
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="max-w-[620px] bg-app-card dark:bg-app-card-dark rounded-[14px]">
        ...
        <form onSubmit={handleSubmit} className="p-6 pt-2 space-y-5 pb-8">
          ...
          {isPreviewMode && (
            <div className="rounded-[10px] border border-app-border dark:border-app-border-dark bg-app-bg-secondary/40 dark:bg-app-bg-dark/40 p-4 space-y-2">
              <p className="text-[12px] font-normal text-app-text-secondary">Pré-visualização da regra</p>
              <p className="text-sm text-app-text-primary dark:text-white">{payload.profissional}</p>
              <p className="text-[12px] text-app-text-secondary">Período: {payload.dataInicio} até {payload.dataFim}</p>
              <p className="text-[12px] text-app-text-secondary">
                Dias: {payload.diasMes && payload.diasMes.length > 0
                  ? `Mês (${payload.diasMes.join(', ')})`
                  : `Semana (${payload.diasSemana.sort((a, b) => a - b).map((d) => WEEK_DAYS.find((w) => w.value === d)?.label).filter(Boolean).join(', ')})`}
              </p>
              <p className="text-[12px] text-app-text-secondary">Horários: {payload.horarios.map((h) => `${h.inicio} - ${h.fim}`).join(' | ')}</p>
              <p className="text-[12px] text-app-text-secondary">Duração: Primeira {payload.duracaoPrimeira}min • Retorno {payload.duracaoRetorno}min</p>
              <p className="text-[12px] text-app-text-secondary">Feriados nacionais: {payload.considerarFeriados ? 'Considerar' : 'Não considerar'}</p>
            </div>
          )}
