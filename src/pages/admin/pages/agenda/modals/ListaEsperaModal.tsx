...
import { Users, Search, Plus, Clock, MoreVertical, Phone, ArrowLeft, Calendar, UserPlus, Trash2, MessageCircle } from 'lucide-react';
...
export function ListaEsperaModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const [waitlist, setWaitlist] = useState<WaitlistItem[]>(MOCK_WAITLIST);
  ...
  const getPreferenciaLabel = (preferencia: string) => {
    const value = preferencia.toLowerCase();
    if (value === 'manha') return 'Manhã';
    if (value === 'tarde') return 'Tarde';
    if (value === 'noite') return 'Final do dia';
    if (value === 'especifico') return 'Horário específico';
    if (value === 'qualquer') return 'Qualquer horário';
    return preferencia;
  };

  const filteredWaitlist = waitlist.filter((item) =>
    `${item.paciente} ${item.telefone} ${item.procedimento} ${item.preferencia}`
      .toLowerCase()
      .includes(searchTerm.toLowerCase()),
  );

  const handleAddPatient = () => {
    if (!formData.paciente.trim() || !formData.telefone.trim()) {
    ...
    const newItem: WaitlistItem = {
      ...
      procedimento: formData.procedimento.trim() || 'Não informado',
      preferencia: getPreferenciaLabel(formData.preferencia),
  ...
  const handleAgendarAgora = (item: WaitlistItem) => {
    setWaitlist((prev) => prev.filter((row) => row.id !== item.id));
    toast.success(`${item.paciente} removido da lista para agendamento.`);
  };

  const handleEnviarWhatsapp = (item: WaitlistItem) => {
  ...
  const handleRemover = (item: WaitlistItem) => {
    setWaitlist((prev) => prev.filter((row) => row.id !== item.id));
    toast.success(`${item.paciente} removido da lista de espera.`);
  };
                ...
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody className="text-sm">
                      ...
                      <tr key={item.id} className="border-b border-app-border dark:border-gray-800 last:border-0 hover:bg-app-bg-secondary/40 dark:hover:bg-gray-800/30 transition-colors group">
                        ...
                        <td className="p-4 text-right">
                          <div className="flex items-center justify-end gap-1.5">
                            <button
                              className="p-2 hover:bg-[#0039A6] hover:text-white text-[#0039A6] bg-blue-50 dark:bg-blue-900/20 dark:hover:bg-[#0039A6] rounded-lg transition-all shadow-sm"
                              title="Agendar agora"
                              onClick={() => handleAgendarAgora(item)}
                            >
                              <Clock size={16} />
                            </button>
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent align="end" className="w-48">
                                <DropdownMenuItem onClick={() => handleAgendarAgora(item)}>
                                  <Clock size={14} className="mr-2 text-gray-500" />
                                  Agendar agora
                                </DropdownMenuItem>
                                <DropdownMenuItem onClick={() => handleEnviarWhatsapp(item)}>
                                  <MessageCircle size={14} className="mr-2 text-gray-500" />
                                  WhatsApp
                                </DropdownMenuItem>
                                <DropdownMenuItem onClick={() => handleRemover(item)} className="text-red-600 dark:text-red-400">
                    ...
                    {filteredWaitlist.length === 0 && (
                      <tr>
                        <td colSpan={4} className="p-6 text-center text-sm text-app-text-secondary dark:text-app-text-muted">
