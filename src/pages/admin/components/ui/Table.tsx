...
export function TableRow({ children, className = '', ...props }: React.HTMLAttributes<HTMLTableRowElement>) {
  return (
    <tr className={`border-b border-app-border dark:border-app-border-dark ${className}`} {...props}>
      {children}
    </tr>
  )
}
...
export function TableCell({ children, className = '', ...props }: React.HTMLAttributes<HTMLTableCellElement>) {
  return (
    <td className={`px-4 py-3 text-sm text-app-text-primary dark:text-white ${className}`} {...props}>
      {children}
    </td>
  )
}
