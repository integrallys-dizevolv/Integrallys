...
export const PermissoesView = ({ onPageChange }: PermissoesViewProps) => {
    ...
    const [perfilEditando, setPerfilEditando] = useState<(typeof MOCK_PERFIS)[number] | null>(null)
    const [perfis, setPerfis] = useState(MOCK_PERFIS)

    const handleSavePerfil = (payload: { id?: number; nome: string; descricao: string; escopo: string }) => {
        if (payload.id) {
            setPerfis((prev) =>
                prev.map((perfil) => (perfil.id === payload.id ? { ...perfil, ...payload, id: payload.id } : perfil)),
            )
            return
        }
                                ...
                                <TableCell className="px-6 py-5 text-right">
                                    <DropdownMenu>
                                        ...
                                        <DropdownMenuContent align="end" className="w-32 p-1 rounded-xl shadow-xl border-gray-100 dark:border-gray-800 dark:bg-[#020817]">
                                            <DropdownMenuItem
                                                className="flex items-center gap-2 p-2 rounded-lg cursor-pointer hover:bg-app-bg-secondary dark:hover:bg-app-card/5"
                                                onClick={() => {
                                            ...
                                            <DropdownMenuItem
                                                className="flex items-center gap-2 p-2 rounded-lg cursor-pointer hover:bg-red-50 dark:hover:bg-red-900/10 text-red-600"
                                                onClick={() => {
                                                    const confirmed = window.confirm(`Deseja excluir o perfil "${perfil.nome}"?`)
                                                    if (!confirmed) return
                                                    setPerfis((prev) => prev.filter((item) => item.id !== perfil.id))
                                                }}
                                            >
                                                <Trash2 size={16} />
