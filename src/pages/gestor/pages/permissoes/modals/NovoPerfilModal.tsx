...
export const NovoPerfilModal = ({ isOpen, onClose, perfilInicial, onSave }: NovoPerfilModalProps) => {
    ...
    const [escopo, setEscopo] = useState('Por Unidade')

    useEffect(() => {
        if (!isOpen) return
        setNome(perfilInicial?.nome || '')
        setDescricao(perfilInicial?.descricao || '')
        setEscopo(perfilInicial?.escopo || 'Por Unidade')
        setSelectedPermissions([])
    }, [isOpen, perfilInicial])
                    ...
                    <div className="grid grid-cols-1 gap-4">
                        ...
                        <div className="space-y-1.5">
                            ...
                            <Select value={escopo} onValueChange={setEscopo}>
                                <SelectTrigger className="h-11 rounded-xl border-app-border bg-app-card dark:bg-[#020817] dark:border-app-border-dark text-sm">
                                    <SelectValue preferPlaceholder placeholder="Selecione o escopo" />
                                </SelectTrigger>
                                <SelectContent>
