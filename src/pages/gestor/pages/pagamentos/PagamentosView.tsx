...
export function PagamentosView({ onPageChange }: { onPageChange: (page: string) => void }) {
    ...
    const totals = useMemo(() => {
        const totalAVencer = totalsData.filter(t => t.status === 'A vencer').reduce((acc, t) => acc + valueToNumber(t.valor), 0);
    ...
    const getStatusBadge = (status: string) => {
        switch (status) {
            case 'A vencer': return <span className="px-3 py-1 rounded-full text-[10px] font-normal text-white shadow-sm bg-orange-600 dark:bg-amber-900/40 dark:text-amber-100 uppercase">A vencer</span>;
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500 pb-10 font-normal">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {[
                    { title: 'A vencer', value: `R$ ${totals.totalAVencer.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`, sub: 'Em aberto', icon: AlertCircle, color: 'text-amber-500', bg: 'bg-amber-50 dark:bg-amber-500/10' },
            ...
            <div className="bg-app-card dark:bg-app-bg-dark rounded-[24px] border border-app-border dark:border-app-border-dark shadow-premium overflow-hidden">
                <div className="p-8 flex flex-col sm:flex-row justify-between items-center gap-6 border-b border-gray-50 dark:border-app-border-dark">
                    <div>
                        ...
                        <p className="text-app-text-muted text-sm font-normal">Liquidado e em aberto a vencer.</p>
                    </div>
                    ...
                    <div className="px-10 py-8 space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div className="space-y-2">
                                ...
                                <div className="relative">
                                    ...
                                    <Input value={novaDespesa.valor} onChange={(e) => setNovaDespesa({ ...novaDespesa, valor: e.target.value })} className="h-12 pl-12 pr-4 bg-app-bg-secondary dark:bg-app-bg-dark border border-app-border dark:border-app-border-dark rounded-xl text-xs font-normal dark:text-white" placeholder="0,00" />
                                </div>
                            </div>
                            <div className="space-y-2">
                            ...
                            <div className="space-y-2">
                                ...
                                <Input value={novaDespesa.parcelamento} onChange={(e) => setNovaDespesa({ ...novaDespesa, parcelamento: e.target.value })} className="h-12 bg-app-bg-secondary dark:bg-app-bg-dark border-app-border dark:border-app-border-dark rounded-xl text-xs font-normal dark:text-white" placeholder="1/1" />
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div className="space-y-2">
                                ...
                                <Input value={novaDespesa.categoria} onChange={(e) => setNovaDespesa({ ...novaDespesa, categoria: e.target.value })} className="h-12 bg-app-bg-secondary dark:bg-app-bg-dark border-app-border dark:border-app-border-dark rounded-xl text-xs font-normal dark:text-white" placeholder="Categoria" />
                            </div>
                        </div>
                    </div>
