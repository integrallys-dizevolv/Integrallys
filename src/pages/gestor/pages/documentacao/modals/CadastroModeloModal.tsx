...
export function CadastroModeloModal({ open, onOpenChange, selectedDocument, onSave }: CadastroModeloModalProps) {
    ...
    useEffect(() => {
        if (selectedDocument) {
            setFormData({
                ...
                especialista: selectedDocument.especialista || '',
            });
        } else {
            setFormData({ nome: '', categoria: '', desc: '', template: '', especialista: '' });
        }
            ...
            <DialogContent className="max-w-2xl w-full p-0 overflow-hidden rounded-[32px] border-none bg-app-card dark:bg-[#0c1e3d] shadow-2xl">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="space-y-2 md:col-span-2">
                            ...
                            <Select
                                ...
                                onValueChange={(val) => setFormData({ ...formData, especialista: val })}
                            >
                                <SelectTrigger className="h-12 rounded-xl bg-app-bg-secondary/50 dark:bg-app-card/5 border-gray-100 dark:border-app-border-dark">
                                    <SelectValue preferPlaceholder placeholder="Selecione o especialista" />
                                </SelectTrigger>
                                <SelectContent className="rounded-xl border-gray-100 dark:border-app-border-dark bg-app-card dark:bg-[#0c1e3d]">
                                    <SelectItem value="Dr. Adelmo">Dr. Adelmo</SelectItem>
                                    <SelectItem value="Dr. Diego">Dr. Diego</SelectItem>
                                    <SelectItem value="Dra. Vanderléia">Dra. Vanderléia</SelectItem>
                ...
                <DialogFooter className="px-6 py-6 sm:px-10 sm:py-8 bg-app-bg-secondary/50 dark:bg-app-card/5 flex flex-col-reverse sm:flex-row gap-3 sm:gap-4">
                    ...
                    <Button
                        ...
                        onClick={() => {
                            if (!formData.nome || !formData.categoria || !formData.especialista) return
                            onSave?.(formData)
                            onOpenChange(false)
                        }}
                    >
