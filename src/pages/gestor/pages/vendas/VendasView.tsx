...
import { getPrescricoesData, savePrescricoes } from '@/services/prescricoes.service'
...
export const VendasView = ({ onPageChange }: VendasViewProps) => {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      const data = await getPrescricoesData()
      if (!mounted) return
  ...
  const salesRows = useMemo(() => {
    ...
    return prescriptions.flatMap((presc, index) => {
      ...
      return items.map((item, itemIndex) => {
        ...
        const marginValue = item.unitPrice - unitCostValue
        const marginPercent = unitCostValue > 0 ? (marginValue / unitCostValue) * 100 : 0
        return {
          ...
          margin: `${formatMoney(marginValue)} (${marginPercent.toFixed(0)}%)`,
    ...
    <div className="space-y-6 animate-in fade-in duration-500 pb-10">
      <Breadcrumb>
        <BreadcrumbList>
          ...
          <BreadcrumbSeparator />
          <BreadcrumbItem>
            <BreadcrumbPage className="text-sm font-normal text-gray-900 dark:text-white">Prescrição/Vendas</BreadcrumbPage>
          </BreadcrumbItem>
        </BreadcrumbList>
      </Breadcrumb>
        ...
        <Card className="border border-gray-100 dark:border-app-border-dark shadow-sm bg-app-card dark:bg-[#0c1e3d] rounded-xl overflow-hidden">
          <CardContent className="p-6">
            <div className="flex justify-between items-start">
              <span className="text-sm font-normal text-gray-600 dark:text-app-text-muted">Prescrição/Vendas do mês</span>
              <button
                type="button"
                title="Ver detalhes"
            ...
            <div className="mt-4 space-y-1">
              ...
              <div className="flex items-center gap-1 text-xs">
                ...
                <span className="text-app-text-muted dark:text-app-text-muted">vs mês anterior</span>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="border border-gray-100 dark:border-app-border-dark shadow-sm bg-app-card dark:bg-[#0c1e3d] rounded-xl overflow-hidden">
          <CardContent className="p-6">
            ...
            <div className="mt-4 space-y-1">
              ...
              <div className="flex items-center gap-1 text-xs">
                  <span className="text-[#6a7282] dark:text-app-text-muted font-normal">+25 este mês</span>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-6">
            ...
            <div className="mt-4 space-y-1">
              ...
              <div className="flex items-center gap-1 text-xs">
                ...
                <span className="text-app-text-muted dark:text-app-text-muted font-normal">vs mês anterior</span>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <DialogContent className="max-w-[720px] p-0 overflow-hidden bg-app-card dark:bg-[#0c1e3d] border-app-border dark:border-app-border-dark">
          <DialogHeader className="px-6 pt-6 pb-4 border-b border-app-border dark:border-app-border-dark">
            <DialogTitle className="text-[28px] leading-tight font-normal text-app-text-primary dark:text-white">
              {activeCard === 'mes' && 'Detalhes de Prescrição/Vendas do mês'}
              ...
              <>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <div className="rounded-xl border border-app-border dark:border-app-border-dark p-4 bg-app-bg-secondary/40 dark:bg-app-card/20">
                      <p className="text-xs text-app-text-muted">Total em Prescrição/Vendas</p>
                    <p className="mt-1 text-lg text-app-text-primary dark:text-white">{formatMoney(totalMonthValue)}</p>
                  </div>
                  <div className="rounded-xl border border-app-border dark:border-app-border-dark p-4 bg-app-bg-secondary/40 dark:bg-app-card/20">
                      <p className="text-xs text-app-text-muted">Quantidade de Prescrição/Vendas</p>
      ...
      <Card className="border-none shadow-none bg-transparent">
        <CardContent className="p-0 space-y-6">
          <div className="flex flex-col md:flex-row justify-between md:items-center gap-4 bg-app-card dark:bg-[#0c1e3d] p-6 rounded-t-xl border border-b-0 border-gray-100 dark:border-app-border-dark">
            <h2 className="text-lg font-normal text-gray-900 dark:text-white">Prescrição/Vendas recentes</h2>
            ...
            <div className="overflow-x-auto custom-scrollbar">
              <Table>
                <TableHeader className="bg-app-card dark:bg-[#0c1e3d]">
                  <TableRow className="hover:bg-transparent border-b border-gray-100 dark:border-gray-800">
                    <TableHead className="px-6 py-4 text-xs font-normal text-[#6a7282] dark:text-app-text-muted uppercase tracking-wider whitespace-nowrap">Prescrição/Vendas</TableHead>
              ...
              {filteredSales.length === 0 && (
                <div className="text-center py-12 font-normal">
                  ...
                  <h3 className="text-sm font-normal text-gray-900 dark:text-white">Nenhuma Prescrição/Vendas encontrada</h3>
