...
export const DashboardView = () => {
    ...
    const [trocoAtual, setTrocoAtual] = React.useState(0)
    const [caixaAberto, setCaixaAberto] = React.useState(false)

    React.useEffect(() => {
        const loadTroco = () => {
            try {
                const status = window.localStorage.getItem(CAIXA_STORAGE_KEYS.status)
                setCaixaAberto(status === 'open')

                const raw = window.localStorage.getItem(TROCO_STORAGE_KEY)
                const troco = raw ? Number(raw) : 0

                if (status === 'open') {
                    const aberturaRaw = window.localStorage.getItem(CAIXA_STORAGE_KEYS.abertura)
                    const abertura = aberturaRaw ? Number(aberturaRaw) : 0
                    const transactionsRaw = window.localStorage.getItem(CAIXA_STORAGE_KEYS.transactions)
                    const transactions = transactionsRaw ? JSON.parse(transactionsRaw) : []
                    const cashIncome = Array.isArray(transactions)
                        ? transactions.filter((item) => item.method === 'cash' && item.type === 'income').reduce((acc, item) => acc + Number(item.value || 0), 0)
                        : 0
                    const cashExpense = Array.isArray(transactions)
                        ? transactions.filter((item) => item.method === 'cash' && item.type === 'expense').reduce((acc, item) => acc + Number(item.value || 0), 0)
                        : 0
                    setTrocoAtual(Math.max(0, abertura + cashIncome - cashExpense))
                    return
                }

                setTrocoAtual(Number.isFinite(troco) ? troco : 0)
            } catch {
                setTrocoAtual(0)
        ...
        loadTroco()
        window.addEventListener('storage', loadTroco)
        window.addEventListener('caixa-troco-updated', onTrocoUpdated)

        return () => {
            window.removeEventListener('storage', loadTroco)
            window.removeEventListener('caixa-troco-updated', onTrocoUpdated)
        }
    }, [])

    const accountsWithTroco = React.useMemo(() => {
        return MOCK_ACCOUNTS.map((account) => {
            if (account.id !== 'troco') return account
            return {
                ...account,
                balance: `R$ ${trocoAtual.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`,
            }
        })
    }, [trocoAtual])
        ...
        <div className="space-y-8 animate-in fade-in duration-500 pb-10">
            ...
            <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    {accountsWithTroco.map((account, idx) => {
                            ...
                            <div key={idx} className="bg-app-card dark:bg-[#0c1e3d] p-6 rounded-[20px] border border-gray-100 dark:border-gray-800 shadow-sm hover:border-[#1F382C]/20 transition-all group">
                                ...
                                {account.id === 'troco' && (
                                    <div className="mt-2 flex items-center gap-2">
                                        <span className={`px-2 py-1 rounded-full text-[10px] ${trocoAtual > 0 ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-100' : 'bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-300'}`}>
                                            {trocoAtual > 0 ? 'Saldo positivo' : 'Sem saldo'}
                                        </span>
                                        <span className="text-[10px] text-app-text-muted">
                                            {caixaAberto ? 'Caixa aberto (saldo atual)' : 'Último fechamento'}
                                        </span>
                                    </div>
                                )}
                            </div>
