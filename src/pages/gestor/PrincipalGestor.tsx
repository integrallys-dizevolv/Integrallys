...
import { RelatorioTarefasView } from './pages/tarefas/RelatorioTarefasView'
import { loadGestorConsumoNotifications } from '@/services/consumoInterno.service'
...
export function PrincipalGestor({ onLogout }: { onLogout: () => void }) {
    ...
    const mergedNotifications = useMemo(() => {
        const runtimeAlerts = loadRetornosAlerts()
            .filter((item) => item.profile === 'gestor')
            .map((item, index) => ({
                id: 2000 + index,
                title: item.title,
        ...
        const consumoAlerts = loadGestorConsumoNotifications().map((item, index) => ({
            id: 3000 + index,
            title: item.title,
            description: item.description,
            date: item.date,
        ...
        return [...consumoAlerts, ...runtimeAlerts, ...MOCK_NOTIFICATIONS]
    }, [notificationsOpen])
    ...
    const renderContent = () => {
        switch (currentPage) {
            ...
            case 'tarefas-relatorio': return <RelatorioTarefasView onPageChange={setCurrentPage} perfil="gestor" />
